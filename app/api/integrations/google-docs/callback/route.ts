...
export async function GET(request: NextRequest) {
  console.log('[GOOGLE DOCS CALLBACK] Route hit');
  ...
  try {
    ...
    const tokenData = await GoogleDocsIntegration.exchangeCodeForToken(code);
    console.log('[GOOGLE DOCS CALLBACK] Token received');
    ...
    return NextResponse.redirect(new URL('/integrations?success=google_docs_connected', appUrl));
  } catch (error) {
